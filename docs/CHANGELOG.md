...
- **v0.6.18**: 用户三缺陷根因修复（viewer 标记 #1~#4：跑道槽端方形、侧边 0.1mm 薄壁未分开、挂耳处全高怪棱）。验收：修复前旧代码 × 新图纸（20260922 出图与 20260820 逐位一致）净差 +7,104.99（+3.70%）= v0.6.17 历史基线 +7,105；修复后 +7,307.69（+3.81%），净 +202.7 为修复的净加材料效应（恢复被多切的圆弧端头/球台/弦棱/臂环弧带，减去棱去除与薄壁扩切），非回归；回归套件 6/6。
