...
| bracket angker（三视图） | 净差 −267.38（−0.14%），重合 **99.0%** | 收敛；v0.6.18 刀组修复连带改善（恢复被多切的弧端/球台/弦棱 → 比 v0.6.17 多留 252；v0.6.17 时 −519.82/−0.27%，v0.6.16 时 −389.77/−0.20%） |
| bracket angker（三视图+剖面图纸） | 净差 +7,307.69（+3.81%），重合 **99.1%** | 用户三缺陷已修复（端头弧/薄壁/挂耳五保护体；净 +202.7 = 修复净加材料效应非回归）。**剩余误差已定位成一块**（2026-09-22 截面叠加图 + 盒探针）：臂区方块 x[96,162]×y[±25] z>27 存活——探针 z[27,33] 重建 2,382.3 vs 基准 1,772.7、z[38,42] 1,588.2 vs 1,181.8，而同区域三视图路径 1,770.1 / 1,180.1 精确；本可把它裁成 R25.5 圆柱的 `sec_B` 棱柱因"10×44 vs 父 51×44 非全尺寸"被跳过、`sec_C` 剖面环提取失败，实际只有 `sec_A`(y=0) 生效 |
| 简单模型回归套件 | 6/6 | 绿 |
