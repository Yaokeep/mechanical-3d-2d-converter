...
版本 v0.6.9（git 最新提交为准；git tag 只到 v0.5.5）。代码内版本字符串与 git 同步：`app.py` = `"0.6.9"`、`main_window.py` 窗口标题 = `"v0.6.9"`、关于对话框 = `"v0.6.9"`：
...
- **版本号同步**: `src/app.py`（`APP_VERSION`）、`src/gui/main_window.py`（`setWindowTitle` 1 处 + 关于对话框 `main_window.py:535` 1 处，共 2 处）、`CLAUDE.md` 和 git tag 四处版本号需同步。当前 git 为 `v0.6.9`（代码内 `0.6.9`）——提交新版本时务必同步更新这些位置。
